 ...
 * @LastEditors: Wang Xi
 * @LastEditTime: 2024-07-16 00:35:54
-->
<template>
    ...
    <Viewer>
      ...
      <div class="table_box">
        ...
        >
          ...
          <el-table-column label="任课教师" prop="teachers" align="center">
            <template #="{ row }">
              {{
                row.teachers
                  ? row.teachers
                      .map((m) => teacherList.find((f) => f.id == m)?.name)
                      .join(",")
                  : ""
              }}
            </template>
    ...
    >
      ...
      >
        ...
        <el-form-item label="容纳最大人数：" prop="max_persons">
          <el-input-number
            v-model="saveCourseForm.max_persons"
            placeholder="请输入容纳最大人数"
            :min="0"
          ></el-input-number>
        </el-form-item>
        <el-form-item label="任课教师：" prop="teachers">
          <el-select
            v-model="saveCourseForm.teachers"
            placeholder="请选择任课教师"
            multiple
            collapse-tags-tooltip
            collapse-tags
...
import { reqGetTeacherList } from "@/api/teacher/teacher";
...
const getList = () => {
  ...
  reqGetCoursesList(searchForm.value).then((res: any) => {
...
const getTeacherList = () => {
  reqGetTeacherList({ pageSize: 9999 }).then((res: any) => {
...
const submitForm = async (formEl: any) => {
  ...
  await saveFormRef.value.validate((valid: boolean) => {
    if (valid) {
      reqSaveCourses(saveCourseForm.value).then((res: any) => {
